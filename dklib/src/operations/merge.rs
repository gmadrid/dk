...
pub fn merge_charts(left: &Chart, right: &Chart) -> Chart {
	...
	throw!(Error::IncompatibleMerge{dimen: "rows", rows1: left.rows().into(), rows2: right.rows().into()});
    }
    //ensure!
    if left.cols() != right.cols() {
	throw!(Error::IncompatibleMerge{dimen: "columns", rows1: left.cols().into(), rows2: right.cols().into()});
    }
